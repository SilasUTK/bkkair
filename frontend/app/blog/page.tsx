      ...
      <section className="bg-white px-6 py-16 lg:px-8 lg:py-24">
        <div className="mx-auto max-w-7xl">
          <div className="max-w-3xl">
            <p className="text-sm font-black uppercase tracking-widest text-[#003d82]">Visa Knowledge</p>
          ...
          <div className="mt-12 grid gap-6 md:grid-cols-2 lg:grid-cols-3">
            {blogPosts.map((post) => (
              <article key={post.slug} className="flex h-full flex-col rounded-[2rem] border border-slate-100 bg-[#F8FAFC] p-6 shadow-sm">
                <p className="text-xs font-black uppercase tracking-widest text-[#f59e0b]">{post.category}</p>
                ...
                <Link href={`/blog/${post.slug}`} className="mt-6 inline-flex items-center font-bold text-[#003d82]">
